...
def res_block_3d(input, out_channels=64, scope='res_block', kernel=[3, 3, 3],
                 prob = 0.5,  stride=[1, 1, 1], weight_dict=None):
...
def res_block_2d(input, out_channels=64, scope='res_block', kernel=[3, 3],
                 prob = 0.5,  stride=[1, 1], weight_dict=None):
    with tf.variable_scope(scope):
...
def conv2d(input_, num_outputs, kernel_size=[4,4], stride=[1,1], pad='SAME',
           if_bias=True, trainable=True, reuse=False, scope='conv2d',
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        conv = tf.nn.conv2d(input_, w, padding=pad, strides=[1] + stride + [1])
...
def conv2d_transpose(x, num_outputs, kernel_size=(4,4), stride=(1,1),
                     pad='SAME', if_bias=True, reuse=False,
                     scope="conv2d_transpose", trainable=True,
                     weight_initializer=None, bias_initializer=None,
                     weight_initializer_type=tf.random_normal_initializer(stddev=0.02)):
    print(scope)
    with tf.variable_scope(scope, reuse=reuse):
        ...
        conv_trans = tf.nn.conv2d_transpose(x, w, output_shape=output_shape,
                                            strides=[1] + stride + [1],
                                            padding=pad)
...
def conv2d_specnorm(input_, num_outputs, kernel_size=[4,4], stride=[1,1],
                    pad='SAME', if_bias=True, trainable=True, reuse=False,
                    scope='conv2d', weight_initializer=None,
                    bias_initializer=None, u_weight=None,
                    weight_initializer_type=tf.random_normal_initializer(stddev=0.02)):
    print(scope)
    with tf.variable_scope(scope, reuse=reuse):
        if weight_initializer is None:
        ...
        conv = tf.nn.conv2d(input_, spectral_norm(w, 1, u_weight=u_weight),
                            padding=pad, strides=[1] + stride + [1])
...
def conv2d_transpose_specNorm(x, num_outputs, kernel_size=(4, 4),
                              stride= (1, 1), pad='SAME', if_bias=True,
                              reuse=False, scope="conv2d_transpose",
                              trainable=True, weight_initializer=None,
                              bias_initializer=None, u_weight=None,
                              weight_initializer_type=tf.random_normal_initializer(stddev=0.02)):
    print(scope)
    with tf.variable_scope(scope, reuse=reuse):
        if weight_initializer is None:
            ...
            print("Loading weights")
            w = tf.get_variable(name='weights', initializer=weight_initializer,
                                dtype=tf.float32, trainable=trainable)
...
def conv3d(input_, num_outputs, pad="SAME", reuse=False, kernel_size=[4, 4, 4],
           stride=[2, 2, 2], if_bias= True, trainable=True, scope="conv3d",
           weight_initializer=None, bias_initializer=None,
           weight_initializer_type=tf.random_normal_initializer(stddev=0.02)):
    print(scope)
    with tf.variable_scope(scope, reuse=reuse):
        ...
        conv = tf.nn.conv3d(input_, w, padding=pad, strides=[1] + stride + [1])
...
def conv3d_transpose(x, num_output, kernel_size=(4, 4), stride=(1, 1),
                     pad='SAME', if_bias=True, reuse=False,
                     scope="conv3d_transpose", trainable=True,
                     weight_initializer=None, bias_initializer=None,
                     weight_initializer_type=tf.random_normal_initializer(stddev=0.02)):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        conv_trans = tf.nn.conv3d_transpose(x, w, output_shape=output_shape,
                                            strides=[1] + stride + [1],
                                            padding=pad)
...
def conv3d_transpose_specNorm(x, num_output, kernel_size=(4, 4), stride=(1, 1),
                              pad='SAME', if_bias=True, reuse=False,
                              scope="conv3d_transpose", trainable=True,
                              weight_initializer=None, bias_initializer=None,
                              u_weight=None,
...
def fully_connected(input_, output_size, scope='fully_connected', if_bias=True,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        if weight_initializer is None:
            ...
            print("Loading weights")
            matrix = tf.get_variable("weights", initializer=weight_initializer,
                                     dtype=tf.float32, trainable=trainable)
